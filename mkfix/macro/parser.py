...
A macro is lines: a statement per line, blocks by indentation, `#` to the
...
def parse(text: str) -> tuple[Macro, list[Diagnostic]]:
    ...
    parser = _Parser(text)
    macro = parser.parse()
    return macro, sorted(parser.diagnostics, key=lambda d: (d.line, d.col))
