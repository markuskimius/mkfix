...
# Which side of an order a block plays. A macro may hold blocks of each.
...
# A macro is for one side of an order, and the UI keeps the two apart: a
# *market* macro answers orders we receive (`on order`); a *client*
# macro sends orders or minds ones sent by hand (`run`, `on sent order`).
MACRO_SIDES = ("client", "market")


def side_of(kind: str) -> str:
    """The macro side a kind of block belongs to."""
...
EVENTS: dict[str, Event] = {e.name: e for e in (
    ...
    Event("error", SIDES, doc="An action of this macro was refused; event.text says why."),
)}
...
STATEMENTS: dict[str, tuple[str, str]] = {
    ...
    "run": ("run [on SESSION]", "A block that sends its own orders; starts when you press Run. Without `on SESSION` the session is chosen at Run…, so one macro can run on several at once."),
...
def scope_schema(names: tuple[str, ...] | list[str] = ()) -> dict[str, Any]:
    ...
    against; ``names`` are the macro's own `let`/`with` names."""
    return {
