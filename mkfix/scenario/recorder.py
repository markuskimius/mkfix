"""Recording: work orders by hand, get the script that would have done it.
...
out in the scenario language, one block an order:
...
recording follows orders sent by hand (from their `new`); orders a scenario
...
# What the counterparty does that a script can wait for, per side.
_HEARD = {
...
class Recorder:
    def __init__(self, engine: FixEngine, side: str, session: str = "",
        ...
        if side not in vocab.SCENARIO_SIDES:
    ...
    def _on_event(self, ev: EngineEvent) -> None:
        ...
        if order is None or (self.session and ev.session_id != self.session):
            return
        if order["direction"] != ("RX" if self.side == "market" else "TX") or order.get("scenario"):
            return
    ...
    async def _body(self, line: _Timeline, since: float) -> list[str]:
        ...
        if heard is not None and self.side == "client":
            # What the counterparty last did is what a client script would check for.
