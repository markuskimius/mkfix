"""Arming scenarios and giving each matching order its script.
...
from .nodes import Action, Scenario
...
class Run:
    ...
    def verdict(self) -> str:
        """failed if any order's script failed, else passed if any passed, else blank."""
...
class ScenarioRunner:
    ...
    def __init__(self, engine: FixEngine, clock: Clock | None = None, *,
        ...
        self._claims: dict[str, Instance] = {}        # tag -> the script whose `new` is going out
    ...
    def arm(self, scenario: Scenario, *, session: str | None = None, seed: int | None = None,
        ...
        if not scenario.blocks:
            raise ScenarioError(f"{scenario.name!r} has no block to run")
        ...
        run = Run(next(self._ids), scenario, seed, speed, session)
    ...
    def start(self, run: Run) -> None:
        ...
        for block in run.scenario.blocks:
            ...
            if any(isinstance(st, Action) and st.verb == "new" for st in block.body) or not contains_new(block.body):
                self._start_script(run, block, block.body, {}, 0)        # the block is one order's script
            else:
    ...
    def _start_script(self, run: Run, block: Any, body: Any, names: dict[str, Any], n: int) -> bool:
        """A sending script, its order still to come. False when the run is full or over."""
        if run.status != "armed":
            return False
    ...
    def stop(self, run: Run) -> None:
        """Disarm: no new orders, and every live script of the run stops where it is."""
    ...
    def _changed(self, instance: Instance) -> None:
        if self.on_change is not None:
            try:
                ...
                log.exception("scenario on_change failed")
    ...
    def _log(self, instance: Instance | None, line: int, text: str, run: Run | None = None) -> None:
        ...
        if self.on_log is not None:
            try:
                ...
                log.exception("scenario on_log failed")
