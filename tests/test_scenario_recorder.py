...
from mkfix.scenario.instance import PASSED
from mkfix.scenario.recorder import Recorder, _duration, _number, _quote, _value

from tests.test_engine import _fetch_all, stack  # noqa: F401
from tests.test_scenario_sending import Pair
...
class TestMarketRecording:
    ...
    async def test_a_symbol_worked_two_ways_is_told_apart_by_quantity_narrowest_first(self, hand):
        ...
        assert scenario.check(source, side="market")[1] == []
    ...
    async def test_what_is_not_recorded(self, hand):
        ...
        hand.pair.arm("scenario auto\non order where symbol == 'AUTO'\n    accept\n")
        await hand.new(symbol="AUTO")                                   # a scenario's order
        ...
        assert scenario.check(result["source"], side="market")[0].blocks == []
...
class TestClientRecording:
    ...
    async def test_a_sent_order_becomes_a_run_block_that_checks_what_it_heard(self, hand):
        ...
        assert body(source) == [
            ...
            "    expect canceled within 2s",
            "    pass"]
        sc, diags = scenario.check(source, side="client")
    ...
    async def test_several_orders_keep_their_spacing_and_their_terms(self, hand):
        ...
        assert scenario.check(source, side="client")[1] == []
...
class TestWriting:
    ...
    def test_any_text_survives_the_round_trip(self, text):
        ...
        sc, diags = scenario.check(source)
    ...
    def test_every_verb_is_recorded_under_its_op(self):
        ...
        from mkfix.scenario.recorder import _VERB_OF
        from mkfix.scenario import vocab
...
class TestThroughTheManager:
    ...
    async def test_fix_cmd_starts_and_stops_a_recording_a_side(self, stack):
        from tests.test_scenario_store import _ask, _manager
        db, writer, engine = stack
        pair = Pair(db, engine)
        ...
        try:
            ...
            assert venue["orders"] == 1 and "scenario my venue\n" in venue["source"]
            saved = await ask("save_scenario", {"name": "my venue", "source": venue["source"], "side": "market"})
