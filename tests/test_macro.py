"""The macro language: parser, checker, vocabulary, and the bundled
...
from mkfix.macro import functions, nodes, vocab
from mkfix.macro.nodes import (
...
EXAMPLES = sorted((ROOT / "mkfix" / "macro" / "examples").glob("*.macro"))
...
def problems(text: str, **known) -> list[tuple[int, int, str]]:
    return [(d.line, d.col, d.message) for d in macro.check(text, **known)[1]]


def clean(text: str, **known):
    sc, diags = macro.check(text, **known)
...
def scenario_free_vocabulary() -> str:
    """Every word the language and its help are built from, as one string."""
    import json
    return json.dumps(macro.vocabulary()).lower()
...
class TestExamples:
    ...
    def test_the_market_side_is_covered(self):
        ...
        macro functions appear in some example — so a new word fails here
        ...
        for path in EXAMPLES:
            sc, _ = macro.check(path.read_text(encoding="utf-8"))
